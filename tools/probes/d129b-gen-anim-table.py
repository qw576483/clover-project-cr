...
为什么必须改（根因，见 `.ai-tmp/test/D129-unit-recon.md` 的 Q2 / `策划/单位帧段表.md:2717-2720`）：
...
* 选取 **`_5`**（该组的第 5 条 clip）。判据 = 本片出的联络图
  `.ai-tmp/test/D129b-view-pick-*.png`（生成器 `tools/probes/d129b-view-pick.py`，逐格标注 `视角号 + frame_NNN`）：
...
* 为什么本工程要 `_5` 而不是 `_9`：客户端朝向用 `SpriteRenderer.flipX`（`UnitView.cs:268`
  ...
  蓝方女枪手是**侧身朝左**（枪管指向 -x，裁切图 `.ai-tmp/test/D129b-ref2-musketeer.png`），
...
------------------------------------
  `--verify <UnitAnimTable.cs>` 断言：
    ...
    ④ C# 的 `StepToView` 必须与 `tools/probes/d129c-view-map.tsv` 的 view 列**逐字一致**。
  · 对**旧文件**跑 → 必须 FAIL（负控：旧表没有 `ViewRuns`，槽数 = 0；且旧数据把 9 个视角并在一起）。
  · 对新生成的文件跑 → 必须 PASS。

复跑
----
  /c/Python312/python tools/probes/d129b-clip-segments.py --root . --dirs all   # 1) 出单 clip 帧段表
  /c/Python312/python tools/probes/d129c-yaw-verify.py --root .                 # 2) 反解并校验朝向→视角表
...
VIEW_COUNT = 9
# 朝向→视角映射表的**唯一权威副本**（由 `d129c-yaw-verify.py` 写/校验，本生成器只读并逐字复制）
VIEW_MAP_TSV = ('tools', 'probes', 'd129c-view-map.tsv')
...
def load_view_map(root):
    """读 `tools/probes/d129c-view-map.tsv` → [view, ...]（16 档顺序）。缺失/不合法 ⇒ 抛错（⛔ 不许静默兜底）。"""
    p = os.path.join(root, *VIEW_MAP_TSV)
    rows = list(csv.DictReader(open(p, encoding='utf-8'), delimiter='\t'))
    if len(rows) != 16:
        raise SystemExit('映射表 %s 应有 16 行，实际 %d 行' % (p, len(rows)))
    rows.sort(key=lambda r: int(r['step']))
    for i, r in enumerate(rows):
        if int(r['step']) != i:
            raise SystemExit('映射表 %s 的 step 列不连续：第 %d 行 step=%s' % (p, i, r['step']))
    return [int(r['view']) for r in rows]
...
namespace CR.View
{
    ...
    public static class UnitAnimTable
    {
        ...
        /// </summary>
        public static readonly int[] StepToView = { %s };
...
def emit(root, by, old, hit_tsv, out_path, log):
    step_to_view = load_view_map(root)
    ...
    L = [HEADER % ', '.join('%d' % v for v in step_to_view)]
...
STEPTOVIEW_RE = re.compile(r'StepToView = \{ ([^}]*?) \};')
...
def verify(path, by, root=None):
    ...
    ④ `StepToView` 必须与 `tools/probes/d129c-view-map.tsv` 的 view 列逐字一致。
    """
    ...
    if root is not None:
        want = load_view_map(root)
        txt = open(path, encoding='utf-8').read()
        m = STEPTOVIEW_RE.search(txt)
        got = [int(x) for x in m.group(1).replace(' ', '').split(',') if x != ''] if m else []
        if got != want:
            fails.append(('(全局)', 'StepToView', 'C# 表 %s != 权威映射表 %s' % (got, want)))
        else:
            n += 1
...
def layout_check(by, log):
    ...
    **严格单调** —— 原版导出把同一动作的 9 个视角按 `_1…_9` 顺序**连排**，所以起点必然递增或递减。
    ...
    这条与像素侧的 `d129c-yaw-verify.py`（A1–A6）**互相独立**：一个看画面、一个看帧号排布。
...
def main():
    ...
    if a.verify:
        ...
        print('[verify] 断言「每档 ViewRuns 有 9 槽、每个非 null 视角的帧段落在同一条 clip 内、'
              '默认 Runs 是已收录视角之一、StepToView == 权威映射表」：%s（检查 %d 项，失败 %d）'
    ...
    # 独立判据：素材打包顺序（不看像素）—— 与 d129c-yaw-verify.py 的像素侧判据互相独立
