...
------------------------------------
  `--verify <UnitAnimTable.cs>` 断言：
    ...
    ④ 权威 = 引擎件 `clover-client-unity-engine/Runtime/Presentation/UnitFacingMap.cs`：必须存在，
       ...
       `tools/probes/d129c-view-map.tsv` **若在盘**则作交叉校验（逐字一致），不在盘则跳过并打印一行说明（⛔ 不静默）；
    ⑤ 被校验的 C# 必须引用引擎表。
...
VIEW_COUNT = 9
# 朝向→视角映射表的**交叉校验**副本（权威 = 引擎件 `ENGINE_FACING_MAP`；本文件在盘时才比对）
...
def load_view_map(root):
    """读 `tools/probes/d129c-view-map.tsv` → [view, ...]（16 档顺序）。仅作**交叉校验**（权威 = 引擎件）；
    缺失/不合法 ⇒ 抛错（⛔ 不许静默兜底）。"""
...
def verify(path, by, root=None):
    ...
    ④ 权威 = 引擎件 `ENGINE_FACING_MAP`：必须存在、`Standard16StepToView` 可解析且长度 = `FACING_STEP_COUNT`；
       该 tsv 若在盘另作交叉校验（逐字一致），不在盘则跳过并打印一行说明（⛔ 不静默）；
    ...
    if root is not None:
        ...
        if not os.path.isfile(eng):
            ...
            if len(got) != FACING_STEP_COUNT:
                ...
                n += 1
            # ④b 交叉校验（可选）：tsv 在盘时逐字比对；不在盘 ⇒ 打印一行说明后跳过（⛔ 不静默）
            tsv = os.path.join(root, *VIEW_MAP_TSV)
            if os.path.isfile(tsv):
                want = load_view_map(root)
                if got != want:
                    fails.append(('(全局)', 'StepToView', '引擎表 %s != 交叉校验表 %s' % (got, want)))
                else:
                    n += 1
            else:
                print('  · 交叉校验跳过（tsv 不在盘）：%s ⇒ 权威取引擎件 %s' % (tsv, eng))
        # ⑤ 被校验的 C# 必须引用引擎表
...
def main():
    ...
    if a.verify:
        ...
        print('[verify] 断言「每档 ViewRuns 有 9 槽、每个非 null 视角的帧段落在同一条 clip 内、'
              '默认 Runs 是已收录视角之一、引擎朝向表为权威（tsv 在盘时交叉校验）、C# 引用引擎表」：'
              '%s（检查 %d 项，失败 %d）'
