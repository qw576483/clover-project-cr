...
namespace CR.UI.Panels
{
    ...
    ///
    /// <para>
    /// <b>LOGO 用原版图，不再用文字</b>：原实现拿 `CrUiStyle.CenteredText("Logo", root, "皇室战争", …)` 当 LOGO
    /// —— 那是自己拼的，不是原版的（用户 2026-09-20：「**ui都是你乱拼的，不是原版的**」）。
    /// 现在用 A 本体的官方 LOGO 图元 `loading_out` 帧 028（520×224「CLASH ROYALE」蓝底金冠，
    /// 索引 `策划/原版UI素材索引.md` §4.2），按 <see cref="CrUiStyle.LogoOfficial"/> 取。
    /// 位置与宽度是**量出来的**（模板匹配，见 `.ai-tmp/test/G1-量取.md` B 段）：
    /// `01_启动页_Logo_1320x2868.jpg` 里 LOGO 的窗口 = x=213..1086（宽 874）、y=117..492，
    /// 归一化到 1080 画布 ⇒ **宽 715px、距屏顶 96px、水平居中**（该参照图与 LOGO 图元的归一化互相关系数
    /// NCC=0.66，同项并排图见 `.ai-tmp/test/g1-match-logo.png`）。
    /// </para>
    ///
    /// <para>
    /// <b>主视觉底 = A 的原版图元（AP2 复核过）</b>：<see cref="ResPaths.BootBackground"/> 指向
    ...
    /// 是**同一部件、同一官方来源、不同游戏版本** ⇒ 登记为允许差异 D-AP2-1
    /// （`.ai-tmp/test/AP2-允许差异.md`）。⛔ 不用网页截图当素材（那是降到 JPEG 的二次品）。
    /// </para>
    ///
    /// <para>
